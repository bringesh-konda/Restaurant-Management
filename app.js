...
const navLinkButtons = document.getElementsByClassName('nav-link');

for(let i = 0; i < navLinkButtons.length; i++)
{
  navLinkButtons[i].addEventListener('click', function(){

    menuContainer.innerHTML = '';

    const category = navLinkButtons[i].textContent.toLowerCase();
    var ans = menu.filter((item)=>{
...
//     menuContainer.innerHTML = '';
//     const searchedItem = document.querySelector('#search-input').value.toLowerCase();
//     var ans = menu.filter((item)=>{
//       const name = item.title.toLowerCase();
//       if(name.includes(searchedItem)){
//         return true;
